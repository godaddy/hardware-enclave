...
- **Use the key without biometrics** (when access policy is set, and the consent gate is hardware-enforced). On macOS the Secure Enclave enforces Touch ID inside `SecKeyCreateSignature` and software cannot bypass this. On Windows there are three sub-cases: (a) the **WebAuthn / SK path** (`enclaveapp-windows-webauthn`) is hardware-enforced — the TPM via NGC will not produce a signature without an OS-mediated Hello gesture; (b) the **legacy Platform-KSP path on a non-Hello host** has `NCRYPT_UI_PROTECT_KEY_FLAG` set on the key, the TPM enforces a hardware UI gate via the legacy CryptUI password dialog, and the agent's user-mode `UserConsentVerifier` check is short-circuited via the `NotAvailable` branch — also hardware-enforced; (c) the **legacy Platform-KSP path on a Hello-enrolled host** (used when sshenc's `--legacy` flag is passed, or for pre-v0.6.44 keys) has the flag dropped to avoid the password dialog firing on top of Hello, and the agent's `UserConsentVerifier` Boolean is the gate — soft-consent, in principle hookable by user-mode code execution. Apps that need hardware-enforced consent on Hello-enrolled Windows hosts must use the SK path (sshenc does so by default since v0.6.44).
...
- **Connect to the agent socket and request signatures** (Type 1). Socket permissions and peer UID verification prevent cross-user access, but same-user access is inherent to Unix domain sockets / Windows named pipes. The defense against unlimited signatures is the access-policy / user-presence gate — Touch ID on macOS, the WebAuthn-Hello path on Hello-enrolled Windows, and the no-Hello legacy Platform-KSP path (where `NCRYPT_UI_PROTECT_KEY_FLAG` keeps the TPM gate intact) are all hardware-enforced. The only soft-consent sub-path is the legacy Platform-KSP path on a Hello-enrolled host (used when sshenc's `--legacy` is passed), where the agent's `UserConsentVerifier` Boolean is in principle hookable. Without any access policy at all, an attacker who can reach the agent gets unlimited signatures regardless of platform.
...
Hardware access policies (Touch ID on macOS, Windows Hello via the WebAuthn / SK path, the legacy CryptUI password dialog on a non-Hello Windows host where `NCRYPT_UI_PROTECT_KEY_FLAG` is set) are the only defense against root — they require physical presence that root cannot fake. The exception is the legacy Platform-KSP path on a Hello-enrolled Windows host (used when sshenc's `--legacy` is passed), where the consent gate is the user-mode `Verified` Boolean from `UserConsentVerifier` and a kernel-capable root can hook it. That sub-path is the only one where root is a meaningfully stronger threat than against macOS SE or the other Windows paths.
...
- **Windows Platform-KSP path: hardware vs. soft consent depends on Hello-availability at keygen.** `key.rs` decides whether to set `NCRYPT_UI_PROTECT_KEY_FLAG` based on `is_hello_available()` at key-creation time. **Hello not enrolled at keygen**: flag is set, the TPM enforces a hardware UI gate via the legacy CryptUI password dialog, and at sign time the agent's `UserConsentVerifier` probe returns `NotAvailable` so the user-mode Boolean check is bypassed -- gate is hardware-enforced. **Hello enrolled at keygen**: flag is dropped (per PR #99 -- otherwise the password dialog would fire on top of Hello, a UX regression and misleading security signal) and the agent's `UserConsentVerifier` `Verified` Boolean is the gate -- soft-consent, in principle hookable by an attacker with code execution as the user. `ui_policy::verify_ui_policy_matches` reads the CNG key's actual UI flag at sign time and refuses keys whose flag doesn't match what the metadata says, catching attacker-planted keys with mismatched policy regardless of which sub-path is in use. Apps that need hardware-enforced consent on Hello-enrolled Windows hosts use `enclaveapp-windows-webauthn` (TPM-via-NGC, Hello gesture enforced outside user-mode); sshenc routes through it by default since v0.6.44.
...
- **`.meta` HMAC sidecar on the keyring backend.** `metadata::save_meta_with_hmac` / `load_meta_with_hmac` write and verify a `<label>.meta.hmac` sidecar keyed by a per-app random HMAC key held in the system keyring (`enclaveapp_keyring::meta_hmac_key`). `enclaveapp-app-storage::ensure_key` verifies the sidecar on Linux and rejects HMAC-mismatched loads with `meta_hmac_verify` — a same-UID attacker who edits `<label>.meta` to flip `BiometricOnly` → `None` without also having keyring access is caught. Apple SE and Windows CNG do not write the sidecar because those chips enforce the access policy at key-creation time; `.meta` tamper changes what the UI displays but cannot bypass the hardware gate. Linux TPM does not write the sidecar either, but for a different reason: the Linux TPM backend does not enforce `AccessPolicy` at sign time (see "Linux TPM backend" below). Extending the HMAC sidecar to Linux TPM is a candidate follow-up. The sidecar is absent for pre-upgrade keys — `load_meta_with_hmac` falls through to `load_meta` verbatim in that case, so existing installs migrate transparently on the next key regeneration.
...
**Linux TPM backend:** `AccessPolicy` is stored in key metadata and displayed to the user, but the Linux TPM backend does not consult it at runtime. TPM child keys are created with empty authorization (`Context::create` is invoked with no `auth_value`), so neither signing nor decryption requires any user interaction:

- `LinuxTpmSigner::sign` (`crates/enclaveapp-linux-tpm/src/sign.rs`) signs every request without prompting.
- `LinuxTpmEncryptor::decrypt` (`crates/enclaveapp-linux-tpm/src/encrypt.rs`) calls `Context::ecdh_z_gen` without an HMAC/auth session, so ECIES decryption is also silent.

A key created with `AccessPolicy::BiometricOnly` on Linux TPM provides no biometric gate — it behaves identically to `AccessPolicy::None` at runtime, for both signing and decryption. Applications that require enforced user presence on Linux should not rely on this backend to supply it.

**Keyring backend:** Exists for Linux without TPM. Strictly weaker than hardware backends. Any same-user process can access the keyring. No biometric enforcement. As with the Linux TPM backend, `AccessPolicy` is stored in metadata but is not consulted at sign or decrypt time — `SoftwareSigner::sign` and `SoftwareEncryptor::decrypt` operate on the loaded private key without user interaction regardless of the recorded policy. The keyring must be running; if not, the app errors rather than falling back to plaintext.
